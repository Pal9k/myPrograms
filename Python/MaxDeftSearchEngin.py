...
def crawl_web(seed,max_depth):
    ...
    deft=1
    noLinksInDeft=0
    while tocrawl and deft<=max_depth+1:
        page = tocrawl.pop()
        deft_cnt[deft-1]=deft_cnt[deft-1]-1
        if page not in crawled:
            links=get_all_links(get_page(page))   
            noLinksInDeft=noLinksInDeft+len(links)
            union(tocrawl, links )
            crawled.append(page)
        if deft_cnt[deft-1] == 0:
            deft_cnt.append(noLinksInDeft)
            noLinksInDeft=0
            deft=deft+1
    return crawled
